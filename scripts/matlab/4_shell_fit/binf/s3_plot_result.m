...
pfix = readtable("/media/microlab/hdd/taproschle/model/3_core_fit/binf/core_parameters.csv");
padj = readtable("/media/microlab/hdd/taproschle/model/2_fia/binf/shell_parameters.csv");
padj.value = Results.xbest';

writetable(padj, "shell_parameters.csv")

data = readtable("/media/microlab/hdd/taproschle/model/reactor_data.csv");
data = data(data.experiment == "binf", :);
...
% Solver call
[t, y] = ode15s(@(t, y) model(t,y,padj.value,pfix), tspan, y0, options);
...
K_S2FL       = padj.value(6);
K_S3SL       = padj.value(7);
K_SLactose   = padj.value(8);
K_SGalactose = padj.value(9);
K_SGlucose   = padj.value(10);
K_SNeu5Ac    = padj.value(11);
K_SFucose    = padj.value(12);
