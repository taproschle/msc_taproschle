...
pfix = readtable("/media/microlab/hdd/taproschle/model/2_fia/bbif/shell_parameters.csv");
padj = readtable("/media/microlab/hdd/taproschle/model/2_fia/bbif/core_parameters.csv");
padj.value = Results.xbest';

writetable(padj, "core_parameters.csv")
...
K_S2FL       = pfix.value(6);
K_S3SL       = pfix.value(7);
K_SLactose   = pfix.value(8);
K_SGalactose = pfix.value(9);
K_SGlucose   = pfix.value(10);
K_SNeu5Ac    = pfix.value(11);
K_SFucose    = pfix.value(12);
