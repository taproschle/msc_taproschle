...
mt1 = [-x1.*(th2-(t29.*th1)./8.0);-x1.*(t2+(t29.*th1)./(th3.*8.0));-x1.*(t3+t29.*th1.*4.223924006036696e-2);-x1.*(t4+t30);x1.*(t2+t3+t4-t29.*th1.*4.153991908225623e-1-th14.*x5);x1.*(th14.*x6-(t29.*th1)./(th7.*8.0));x1.*(th14.*x7-(t29.*th1)./(th8.*8.0));x1.*(t4-t30)];
mt2 = [x1.*(t3-t29.*th1.*2.593753788511239e-1);t2.*x1];
Xdot = [mt1;mt2];
