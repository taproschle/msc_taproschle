...
mt1 = [-x1.*(th2-(t29.*th1)./8.0);-x1.*(t2+(t29.*th1)./(th3.*8.0));-x1.*(t3+(t29.*th1)./(th4.*8.0));-x1.*(t4+t29.*th1.*2.500000059310496e-3);x1.*(t2+t3+t4-th14.*x5-(t29.*th1)./(th6.*8.0));x1.*(th14.*x6-(t29.*th1)./(th7.*8.0));x1.*(th14.*x7-(t29.*th1)./(th8.*8.0));x1.*(t4-t29.*th1.*2.500000041938851e-3)];
mt2 = [x1.*(t3-(t29.*th1)./(th10.*8.0));t2.*x1];
Xdot = [mt1;mt2];
